...
- (void)peerConnectionGetStats:(nonnull NSString*)trackID
  ...
  if (!trackID || !trackID.length || (track = self.localTracks[trackID]) ||
...
- (void)peerConnection:(RTCPeerConnection*)peerConnection
  ...
  FlutterEventChannel* eventChannel = [FlutterEventChannel
      eventChannelWithName:[NSString stringWithFormat:@"FlutterWebRTC/dataChannelEvent%1$@%2$d",
                                                      peerConnection.flutterId,
  ...
  if (eventSink) {
    eventSink(
        @{@"event" : @"didOpenDataChannel", @"id" : dataChannelId, @"label" : dataChannel.label});
  }
}
