...
import com.cloudwebrtc.webrtc.fork.BaseBitrateAdjuster;
import com.cloudwebrtc.webrtc.fork.BitrateAdjuster;
import com.cloudwebrtc.webrtc.fork.DynamicBitrateAdjuster;
import com.cloudwebrtc.webrtc.fork.FramerateBitrateAdjuster;
import com.cloudwebrtc.webrtc.fork.H264Utils;
import com.cloudwebrtc.webrtc.fork.HardwareVideoEncoder;
import com.cloudwebrtc.webrtc.fork.MediaCodecUtils;
import com.cloudwebrtc.webrtc.fork.MediaCodecWrapperFactoryImpl;
import com.cloudwebrtc.webrtc.fork.VideoCodecMimeType;
...
public class HardwareVideoEncoderFactory implements VideoEncoderFactory {
  ...
  public VideoEncoder createEncoder(VideoCodecInfo input) {
    ...
    /*if (type == VideoCodecMimeType.H264) {
      Boolean isHighProfile = false;
      try {
        isHighProfile = H264Utils.isSameH264Profile(
                input.params, MediaCodecUtils.getCodecProperties(type, true));
        if(isHighProfile == null) isHighProfile = false;
      } catch (Exception e) {
        e.printStackTrace();
      }
      Boolean isBaselineProfile = false;
      try {
        isBaselineProfile = H264Utils.isSameH264Profile(
                input.params, MediaCodecUtils.getCodecProperties(type, false));
        if(isBaselineProfile == null) isBaselineProfile = false;
      } catch (Exception e) {
        e.printStackTrace();
      }
  ...
  private boolean isHardwareSupportedInCurrentSdkH264(MediaCodecInfo info) {
    ...
    return  (name.contains("OMX.google.") || name.contains("OMX.SEC.") || name.contains("c2.android")) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP;
    //return (name.startsWith(QCOM_PREFIX) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT)
