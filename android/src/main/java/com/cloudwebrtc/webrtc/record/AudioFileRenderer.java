...
class AudioFileRenderer implements SamplesReadyCallback {
    ...
    private void initAudioEncoder(JavaAudioDeviceModule.AudioSamples audioSamples) throws IOException {
    ...
    private void processAudioSamples(JavaAudioDeviceModule.AudioSamples audioSamples) {
        if (audioEncoder == null || !encoderStarted || !isRunning) {
            return;
        }
    ...
    private void drainAudioEncoder() {
        if (audioEncoder == null || !encoderStarted) {
            return;
        }
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                // Expected for an encoder - this is where we get the format
    ...
    void release() {
        ...
        audioThreadHandler.post(() -> {
            try {
                if (audioEncoder != null && encoderStarted) {
                    try {
                        // Signal end of stream only if encoder is in running state
                        ...
                        drainAudioEncoder(); // Drain remaining data
                    } catch (Exception e) {
